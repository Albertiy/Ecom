...
public class OrderDao {
    ...
    * */
    ...
    public OrderPageBean<Order> findUnfilledOrders2(String sid, OrderPageBean<Order> orderPageBean) throws SQLException {
        //搜索参数
        String search = (orderPageBean.getSearch().trim().equals("")?"":orderPageBean.getSearch());
        //排序参数
        String sort = orderPageBean.getSort();
        ...
        try {
            conn = JdbcUtils.getConnection();
            //为了第二次计数total查询，先不写入查询返回内容
            sql = " from _order,users where _order.sid = ? and (_order.state = '2')";
            if(!search.equals("")){//搜索功能
                sql += " and (";
                sql += " _order.create_time LIKE binary '%"+search+"%'";
                sql += " or _order.pay_time LIKE binary '%"+search+"%'";
    ...
    public OrderPageBean<Order> findUnfilledOrders3(String sid, OrderPageBean<Order> orderPageBean) throws SQLException {
        //搜索参数
        String search = (orderPageBean.getSearch().trim().equals("")?"":orderPageBean.getSearch());
        //排序参数
        String sort = orderPageBean.getSort();
        ...
        try {
            conn = JdbcUtils.getConnection();
            //为了第二次计数total查询，先不写入查询返回内容
            sql = " from _order,users where _order.sid = ? and (_order.state = '3')";
            if(!search.equals("")){//搜索功能
                sql += " and (";
                sql += " _order.create_time LIKE binary '%"+search+"%'";
                sql += " or _order.pay_time LIKE binary '%"+search+"%'";
